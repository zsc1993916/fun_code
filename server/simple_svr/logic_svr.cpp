    ...
    while(0 == g_iIsQuit) {
        ...
        for(int index = 0; index < iEventCnt; index ++) {
            ...
            if(iServerFd == events[index].data.fd) {
                ...
                int iClientFd = accept(iServerFd, (struct
                            sockaddr*)&stClientAddr, &uiClientAddrLen);

                pCon = new connection_info();
                pCon->m_iFd = iClientFd;
                inet_ntop(AF_INET, &(stClientAddr.sin_addr), pCon->m_szClientIP,
                        sizeof(pCon->m_szClientIP));
